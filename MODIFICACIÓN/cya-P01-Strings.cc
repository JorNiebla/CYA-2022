...
int main(int argc, char **argv) {
  ...
  if (argc > 1 && std::string(argv[1]) == "--help") {
    ...
    << "Siendo opcode la operación usada 1:Longitud, 2:Inversa, 3:Prefijos, 4:Sufijos, 5:Subcadenas, 6:Prefijos-Sufijos\n";
    return 0;
  }
  if (argc != 4) {
    std::cout 
  ...
  if (filein.is_open() && fileout.is_open()) {
    ...
    while (std::getline(filein,linea)) {                //Lee todas las lineas del fichero 1 a 1.
      ...
      // Cadena c = alf.pertenece(cadena_data);          
      ...
      if (c.longitud() == 0) {                        
        ...
        case 5: //Subcadenas
          ...
          break;
        case 6: //Prefijos menos sufijos
          for (Cadena pr : c.prefijosMenosSufijos()) {
            fileout << pr << " ";
          }
          break;
        default:
