...
void Analyzer::searchstatement(std::string linea, int n) {
  std::regex r_statement("\\s*(for|while)\\s*\\(.*\\)\\s*\\{.*\\}");
  std::regex r_foropen("\\s*for\\s*\\(.*\\)\\s*\\{[^\\}]*");
  std::regex r_forclose("\\}");
  std::smatch matches;
  if (!activeform_) {
    if (std::regex_search(linea,matches,r_statement)) {
      activefor_ = true;
      statements_.push_back(std::make_tuple(n, "LOOP",matches[1].str()));
    } else if (std::regex_search(linea,matches,r_foropen)) {
      statements_.push_back(std::make_tuple(n, "LOOP",matches[1].str()));
      activefor_ = true;
      activeform_ = true;
    }
    ...
    activefor_ = true;
    if (std::regex_search(linea,matches,r_forclose)) {
      activeform_ = false;
    }
  }
...
void Analyzer::analyzeFile(std::istream& input) {
  ...
  while (std::getline(input,linea)) {
    ...
    if (activefor_ || activeform_) {
      activefor_ = false;  
    } else {
      codewithoutfor_ += (linea + '\n');
    }
    n++;
  }
}
...
void Analyzer::printcopy(std::ostream& output) {
  output << codewithoutfor_;
}